...
def calculate_vf_aggregator_surplus(coalition_ids, data, alpha0, beta0, C_fixed, C_var, delta_commission):
    """
    Calculates the net value available FOR FARMERS in coalition S,
    where aggregator commission (delta) is applied ONLY to the surplus
    above the sum of standalone payoffs.
    This is the new v_F(S) function.
    """
    if not coalition_ids:
        return 0.0

    # Ensure coalition_ids is a list
    if not isinstance(coalition_ids, list):
        coalition_ids = list(coalition_ids)

    # 0. Get coalition data and sum of standalones
    coalition_data = data[data['Farmer_ID'].isin(coalition_ids)]
    if coalition_data.empty: return 0.0
    sum_standalone_payoffs_R_S = coalition_data['Standalone_Payoff_INR'].sum()

    # 1. Calculate Gross Potential Value
    V_potential = characteristic_function_v(coalition_ids, data, alpha=alpha0, beta=beta0)

    # 2. Calculate Aggregator Costs
    C_A = C_fixed + C_var * len(coalition_ids) if coalition_ids else 0.0

    # 3. Calculate Net Available Value (after costs)
    V_net_available = max(0.0, V_potential - C_A)

    # 4. Calculate Surplus above Baseline Sum
    surplus_above_baseline = max(0.0, V_net_available - sum_standalone_payoffs_R_S)

    # 5. Calculate Aggregator Profit (Commission on Surplus)
    aggregator_profit_piA = delta_commission * surplus_above_baseline

    # 6. Calculate Net Value for Farmer Distribution (v_F)
    v_F = V_net_available - aggregator_profit_piA

    # Sanity check: v_F should not be less than baseline if V_net > baseline
    # It can be less if V_net itself is less than baseline due to high costs
    # It equals V_net_available if surplus is zero.

    return v_F

# --- Simulation Parameters ---
# Keep parameters the same for comparison, unless specified otherwise
N_FARMERS = 12 # Or 250
SEED = 24004
DATA_FILE = './data/synthetic/indian_farmers_carbon_market_250.csv'
PLOT_DIR = './logs/plots/aggregator_surplus_model/' # New directory for results
os.makedirs(PLOT_DIR, exist_ok=True)
AGG_DIR = './logs/aggregator_surplus_model/' # New directory for results
os.makedirs(AGG_DIR, exist_ok=True)

ALPHA0 = 1.25 # Base potential > 1
BETA0 = 0.0   # No base synergy (focus on alpha0 effect + costs/commission)

C_FIXED = 10000
C_VAR = 1500

# DELTA_VALUES = np.linspace(0.0, 0.50, 11) # Commission 0% to 50%
DELTA_VALUES = np.linspace(0.0, 1.00, 21) # Commission 0% to 50%
# DELTA_VALUES = [0.25]

EXACT_SHAPLEY_THRESHOLD = 10
SHAPLEY_SAMPLES = 1000 * N_FARMERS if N_FARMERS > EXACT_SHAPLEY_THRESHOLD else None
...
if __name__ == "__main__":
    ...
    sum_standalone_payoffs_R_N = data['Standalone_Payoff_INR'].sum() # Precompute for grand coalition

    results_list = []
    print(f"\n[+] Running Aggregator (Surplus Commission) Model Simulation for N={N_FARMERS}")
    ...
    if PERFORM_CORE_CHECK:
        ...
        for delta_commission in DELTA_VALUES:
            v_func_current_delta = functools.partial(
                calculate_vf_aggregator_surplus, # Use the NEW function
                data=data,
                alpha0=ALPHA0,
                beta0=BETA0,
                C_fixed=C_FIXED,
    ...
    for delta_commission in DELTA_VALUES:
        ...
        # --- Define the specific value function v_F for this delta ---
        v_func_current_delta = functools.partial(
            calculate_vf_aggregator_surplus, # Use the NEW function
            data=data,
            alpha0=ALPHA0,
        ...
        # --- Calculate Shapley Values based on v_F ---
        ...
        # --- Calculate Aggregator Profit for N using NEW logic ---
        ...
        surplus_above_baseline_N = max(0.0, V_net_available_N - sum_standalone_payoffs_R_N)
        aggregator_profit_piA = delta_commission * surplus_above_baseline_N # Commission on surplus
        total_farmer_value_vfN = V_net_available_N - aggregator_profit_piA # Farmers get remainder

        # --- Check Core Stability (if N is small) ---
        ...
        results_list.append({
            ...
            'aggregator_profit_piA': aggregator_profit_piA, # Now based on surplus
            'total_farmer_value_vfN': total_farmer_value_vfN, # Now based on surplus commission
            'V_potential_N': V_potential_N,
            'C_A_N': C_A_N,
            'V_net_available_N': V_net_available_N,
            'Surplus_N': surplus_above_baseline_N, # Add surplus for info
            'R_N': sum_standalone_payoffs_R_N, # Add baseline sum for info
    ...
    print("\n[+] Simulation Results Summary (Surplus Commission Model):")
    display_cols = ['delta_commission', 'avg_farmer_payoff', 'avg_abs_gain',
                    'ir_met_percentage', 'aggregator_profit_piA', 'Surplus_N',
                    'gini_coefficient', 'is_in_core']
    print(results_df[display_cols].round(2))

    # Save results
    csv_filename = os.path.join(AGG_DIR, f'aggregator_surplus_model_results_n{N_FARMERS}_delta.csv')
    ...
    plotit()
    print(f"\n[+] Aggregator (Surplus Commission) model analysis complete. Total time: {time.time() - start_time_total:.2f}s")
    # --- End Python Code ---
    
# python 05_v2_AggAsPlayer.py > logs/aggregator_surplus_model/agg_asplayer_n_5.log 2>&1 &
# python 05_v2_AggAsPlayer.py > logs/aggregator_surplus_model/agg_asplayer_n_15.log 2>&1 &